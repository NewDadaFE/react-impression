...
- 修复 **InputGroup 组件** 内嵌 Select 组件时的显示问题
- 修复 **Input 组件** 不支持传入其它原生 dom 属性的问题
- 修复 **LoadingAddon 组件** type 不支持 circle 的问题
- 修复 **Notification 组件** 文字显示不换行的问题
- 修复 **Select 组件** 中 SelectOption 没有默认行高的问题
- 修复 **Select 组件** Table 中的 select 背景样式
- 修复 **Select 组件** 多选情况下 value 为数时的问题
