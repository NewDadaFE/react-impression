...
const TreeSelect = props => {
  ...
  const dimensionMinusTo1 = useCallback(
    (parentNode, treeNode, renderList = [], level = 0, newExpandIds) => {
  ...
  const diffSublingNode = useCallback(
    (node, newcheckedIds) => {
      ...
      let childrenId = []
      let checkedType = 'none' // 兄弟节点被选的情况 none | some | all
  ...
  const CIATSCS = useCallback(
    nextCheckedIds => {
      ...
      const nextCheckedLabelListStrategy = []
      ...
      nodeRenderList.forEach(item => {
        ...
        if (
          ...
          ((filterTreeNode && filterTreeNode(filterString, item)) ||
            item.label.includes(filterString))
        ) {
          if (item.level <= markedLevel) {
            markedLevel = -1
          }
  ...
  const NewSwitcherIcon = useCallback(
    props => {
      ...
      const otherProps = __rest(props, ['id'])
      return (
        <div {...otherProps}>
          {loadingNode === id ? (
